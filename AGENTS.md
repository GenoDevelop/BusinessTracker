...
- Keep list screens compact through the shared theme: table headers use the shared 36-pixel height, rows use a 34-pixel minimum height, and shared icon buttons own their dimensions. Do not apply smaller local `Width`/`Height` values to action buttons because that clips their vector icons; adjust only contextual margins when a toolbar or action column needs tighter spacing.
