...
public sealed class UpdateOrderDataValidator : AbstractValidator<UpdateOrderData>
{
    public UpdateOrderDataValidator()
    {
        RuleFor(x => x.OrderDate).NotEmpty().WithMessage("Data zamówienia jest wymagana.");
        RuleFor(x => x.Status).IsInEnum().WithMessage("Status zamówienia jest nieprawidłowy.");
